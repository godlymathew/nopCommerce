    ...
    public partial class Task
    {
        ...
        public void Execute(bool throwException = false, bool dispose = true, bool ensureRunOnOneWebFarmInstance = true)
        {
            ...
            try
            {
                ...
                if (ensureRunOnOneWebFarmInstance)
                {
                    ...
                    if (nopConfig.MultipleInstancesEnabled)
                    {
                        ...
                        if (scheduleTask != null)
                        {
                            ...
                            {
                                //get expiry time
                                var expiryInSeconds = scheduleTask.Seconds <= 300 ? scheduleTask.Seconds - 1 : 300;
                                ...
                                if (!redisWrapper.PerformActionWithLock(scheduleTask.Type, TimeSpan.FromSeconds(expiryInSeconds), executeTaskAction))
                                    return;
                            }
                            else
                            {
                ...
                //execute task in case if is not yet executed
                if (!taskExecuted)
                {
